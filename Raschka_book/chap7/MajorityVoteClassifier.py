...
class MajorityVoteClassifier(ClassifierMixin, BaseEstimator):
    def __init__(self, classifiers, vote='classlabel', weights=None):
        self.classifiers = classifiers      # list of estimators
        self.named_classifiers = {key: value for key, value in _name_estimators(classifiers)}
        self.vote = vote        # 'classlabel' or 'probability' 
        self.weights = weights      # weight vector

    # X: training data: [N_examples, N_features], y: class-label: [N_examples]
    def fit(self, X, y):
    ...
    def predict(self, X):
        ...
        else:       # 'classlabel' case
            # main algorithm
            # predictions: probability of class-label
            # maj_vote: weighted majority voting
            predictions = np.asarray([clf.predict(X) for clf in self.classifiers_]).T
            maj_vote = np.apply_along_axis(lambda x: np.argmax(np.bincount(x, weights=self.weights)), axis=1, arr=predictions)

        # column_number to class-label
